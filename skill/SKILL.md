...
create_project  ->  write_file  ->  execute  ->  finish
   draft/         обмазка при      запуск      копия в
   с git          сохранении                   clean/
```

- `create_project(base)` заводит `<base>/draft/` — обычное хранилище git.
...
- `finish(base)` копирует черновик в `<base>/clean/`.

> **Проект прежнего выпуска.** Раньше каталоги назывались `черновик` и
> `чистовик`. `create_project` открывает такой проект как есть, а не заводит
> рядом пустой `draft/`; в ответе тогда стоят `"legacy_layout": true` и
> `legacy_note` с тем, что делать дальше. Читайте ответ: пути в нём настоящие, а
> не выведенные из имени каталога.
