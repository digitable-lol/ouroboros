...
#: Строка вида `ключ: значение` в шапке страницы.
...
def front_matter_problems(path: Path) -> list[str]:
    ...
    problems: list[str] = []
    for lineno, line in enumerate(parts[1].splitlines(), 2):
        ...
        if ": " in value:
            problems.append(
                ...
                "двоеточие с пробелом — YAML такую шапку не разберёт; возьмите "
...
def main() -> int:
    ...
    print(f"Страницы документации: шапки разбираются, ссылок проверено {checked} — "
          "все ведут в место.")
    return 0
