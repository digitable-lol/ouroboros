...
1. **В файле без суффикса нет кириллицы.** Кроме закрытого списка слов, которые
   стоят там намеренно: подпись переключателя и настоящие имена каталогов на
   диске (`черновик`, `чистовик` — это `DRAFT_DIRNAME`/`CLEAN_DIRNAME` в
   `ouroboros/sandbox/project.py`, перевести их значит изменить поведение
   инструмента, а не документацию).
...
ALLOWED = {
    ...
    "черновик", "черновике", "черновика", "черновику", "черновиком",
    "чистовик", "чистовике", "чистовика", "чистовику", "чистовиком",
}
...
PENDING: dict[str, str] = {
    "skill/SKILL.md": "навык для ИИ-агента, 555 строк; переводится вместе с навыком, не с документацией",
    "design/brief.md": "исходное задание заказчика — исторический документ, переписывать его нельзя",
    ...
    "scripts/measure/trace-help/README.md": "журнал опыта «помогает ли трасса», 582 строки — запись прогонов, а не страница",
...
def selftest() -> int:
    ...
    cases: list[tuple[str, str, str, bool]] = [
        ...
        (
            "английская страница с именами каталогов на диске",
            "a.md",
            "**English** · [Русский](a.ru.md)\n\n# A\n\nThe draft lives in черновик.\n",
            False,
        ),
        (
