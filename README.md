...
| [`tests/`](tests/) | проверок: <!--state:tests-->580<!--/state--> |
...
Версия <!--state:version-->0.3.0<!--/state-->. Проверки: <!--state:tests-->580<!--/state--> из <!--state:tests-->580<!--/state-->, покрытие с ветвями <!--state:coverage_percent-->94<!--/state--> % (`ruff`, `mypy --strict`, `pytest`). Установка
...
**Одинаковость поведения проверяется, а не предполагается.** 79 программ на пяти
языках прогоняются дважды — без обмазки и с обмазкой — и три исхода сверяются:
вывод, код возврата, исключение. Расхождений было 16, стало 0. Случаи, которые
...
Инструмент обмазывает **сам себя**: 19 файлов, 206 функций, ни одной поломки, и
обмазанная копия проходит все 580 проверок.
