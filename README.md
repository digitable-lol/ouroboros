...
| [`tests/`](tests/) | проверок: <!--state:tests-->835<!--/state--> |
...
| [`SPEC.md`](SPEC.md) | договор о формате записи, общий для шести языков |
...
| [Языки](docs/languages.md) | шесть языков, чем они отличаются в записи |
...
Версия <!--state:version-->0.4.0<!--/state-->. Проверки: <!--state:tests-->835<!--/state--> из <!--state:tests-->835<!--/state-->, покрытие с ветвями <!--state:coverage_percent-->99<!--/state--> % (`ruff`, `mypy --strict`, `pytest`). Установка
...
Все шесть языков прогнаны по отдельности.

**Одинаковость поведения проверяется, а не предполагается.** 102 программы на
шести языках прогоняются дважды — без обмазки и с обмазкой — и три исхода
...
Инструмент обмазывает **сам себя**: 21 файл собственного исходника, 227 функций,
ни одной поломки разбора, и обмазанная копия проходит 834 проверки из 835.
