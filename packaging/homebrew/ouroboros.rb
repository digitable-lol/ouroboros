...
#
# ПРАВКА ЗДЕСЬ ДО ЧЕЛОВЕКА НЕ ДОХОДИТ, пока копию не выложили: brew смотрит в
# хранилище формул, а не в это дерево. Забыть об этом легко и незаметно — у
# соседнего flang кран так отстал от выпусков 0.7.4—0.7.9, и всё это время
# brew install молча ставил 0.7.3. Поэтому за копией следит машина:
# scripts/check_release_published.py сверяет выложенную формулу с этой побайтово
# и отдельно — версию, которую она раздаёт. Проверка висит в scripts/qa.sh и
# ежедневно в .github/workflows/packaging-live.yml.
#
# ЧТО ПРОСТАВЛЕНО. url и sha256 — настоящие, от выпуска v0.6.1. Отпечаток
# посчитан с того самого архива, который отдаёт GitHub, и сверен двумя загрузками:
#
#     curl -sL https://github.com/digitable-lol/ouroboros/archive/refs/tags/v0.6.1.tar.gz | sha256sum
#
...
# digitable-lol/tap, затем короткая строка выше — она подключила хранилище формул
# сама, поставила восемнадцать зависимостей и пакет; затем brew test; затем
# обмазка настоящего файла на Python, запуск (вывод до и после совпал побайтово),
# чтение записей и ответ ouroboros-mcp на initialize.
#
# ДВЕ ВЕЩИ, О КОТОРЫЕ СПОТЫКАЮТСЯ ПРИ УСТАНОВКЕ. Обе — не в формуле, но узнают о
# них через неё:
#
#   1. Правило insteadOf в ~/.gitconfig, переписывающее https://github.com/ в
#      git@github.com:, роняет brew на клонировании хранилища формул —
#      «Permission denied (publickey)». Сам архив исходников (url ниже) при этом
#      берётся обычной загрузкой по https, и правило его не трогает: ломается
#      ровно шаг клонирования, который делает brew, а не формула. Обойти изнутри
#      формулы нельзя. Подставить GIT_CONFIG_GLOBAL=/dev/null тоже нельзя: brew
#      вычищает окружение перед вызовом git (Homebrew/bin/brew, ENV_VAR_NAMES).
#      Лечится заменой insteadOf на pushInsteadOf — см. docs/install.md.
#   2. С Homebrew 6.0 чужие хранилища формул требуют согласия, и в выводе
#      мелькает «Skipping digitable-lol/tap because it is not trusted».
#      Установка при этом идёт: полное имя формулы в командной строке brew
#      считает согласием на неё. Чтобы согласие распространялось на всё
#      хранилище — brew trust digitable-lol/tap.
#
# Имена в этом файле сверяются с тем, что пакет действительно кладёт: см.
# scripts/check_packaging_names.py, она висит в scripts/qa.sh. Проверка заведена
# потому, что здесь однажды стояла строка
# assert_path_exists bin/"ouroboros-mcp-router" — имени, которого никогда не было.
#
class Ouroboros < Formula
  ...
  url "https://github.com/digitable-lol/ouroboros/archive/refs/tags/v0.6.1.tar.gz"
  sha256 "6ad63e354bdf6968d0ace77baf2918e2b263b68f5a0b31cae7e3980049801e80"
