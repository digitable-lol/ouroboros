...
Дальше файлы кладут в черновик через `ouroboros write`, а запускают через
`ouroboros execute`. Оригинал остаётся нетронутым.
...
**Имена доводов есть не везде.** Тут языки расходятся, и это надо знать:
...
У Python и JavaScript имя позиционного довода из записи не восстановить —
смотрите подпись функции в исходнике. У C, C++ и Elixir имена берутся из
разобранной подписи и пишутся прямо в запись. Подробнее — [Языки](languages.md).
