...
[English](measurements.md) · **Русский**

[← к оглавлению](index.ru.md)
...
Все восемь строк сняты одним прогоном `scripts/measure/run.sh` — почему таблицу
нельзя собирать из нескольких, сказано выше.
...
а не содержимым** ([почему](limits.ru.md#плата-за-безопасность-строки-в-c-и-c)).
...
Всё выше — про цену записи. Этот раздел про то, что за неё покупается: **дать
языковой модели чужую программу, спросить, что она сделала на самом деле, и
увидеть, много ли добавляет запись по сравнению с одним лишь исходником.**
...
мерой, объявленной до первого прогона.
