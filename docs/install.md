...
<!--state:version-->0.5.0<!--/state-->: `plugin add` по адресу выше, `list all`
(печатает пять версий, от `0.2.0` до `0.4.0`), `install 0.4.0` — после чего
наружу вынесены ровно два имени, а поставленный `ouroboros` обмазал настоящий
файл **на Go** (метод с получателем по указателю и кириллическими именами),
файл собрался, запустился и дал тот же вывод, что до обмазки, и записи
прочитались: три вызова, ноль испорченных строк, ноль незавершённых.

> **Если у вас в `~/.gitconfig` есть правило `insteadOf`,** переписывающее
> адреса `https://github.com/` на `git@github.com:` — обычное дело у тех, кто
> сам пушит в эти хранилища, — то `asdf plugin add` по открытому адресу упадёт
> на «Permission denied (publickey)»: клонирует его сам `asdf`, и правило
> переписывает адрес до того, как он до нас доходит. Свои обращения к git плагин
> от этого правила уже защищает (`GIT_CONFIG_GLOBAL=/dev/null` в `bin/list-all`),
> но клон плагина делает `asdf`, а не мы. Обход: временно снять правило либо
> клонировать плагин вручную. То же самое касается `brew tap`.
