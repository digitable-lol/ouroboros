...
(печатает шесть версий, от `0.2.0` до `0.5.0`), `install 0.5.0` — после чего
наружу вынесены ровно два имени, а поставленный `ouroboros` обмазал настоящий
файл **на Java**, файл собрался, запустился и дал тот же вывод, что до обмазки,
и записи прочитались: три вызова, ноль испорченных строк, ноль незавершённых.
