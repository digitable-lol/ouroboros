...
## Про отчёт о добыче примеров

Навык
[`skill/SKILL.md`](https://github.com/digitable-lol/ouroboros/blob/main/skill/SKILL.md)
описывает другой отчёт — тот, что возвращает мост `fts_extract_examples`: сколько
вызовов стало примерами для спецификации, сколько отброшено и по каким причинам
(`skipped_by_reason`), с пометками о происхождении вида `// trace:`.

**Моста в этом хранилище нет**, и такого отчёта инструмент не печатает. Разбор
причин отказа оставлен на странице [Границы](../limits.md), потому что сами
причины полезно знать: они объясняют, почему запись бывает непригодна для
машинного вывода, даже когда она есть.

Одно правило оттуда стоит перенести сюда как есть: **пустой счёт отказов — повод
проверить настройку, а не радоваться.** Ровно так же и здесь: `calls_parsed: 0`,
`in_flight: []` и «ошибок не видно» на настоящей нагрузке чаще означают, что
обмазана не та функция или прогон был не тот.
