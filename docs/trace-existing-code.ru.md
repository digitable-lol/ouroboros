...
[English](trace-existing-code.md) · **Русский**
...
Всё показанное ниже — вывод настоящих прогонов.
...
```json
{"ok": true, "base": "/srv/tmp/разбор", "draft": "/srv/tmp/разбор/draft", "clean": "/srv/tmp/разбор/clean"}
```

Путь любой ваш; внутри инструмент сам заводит `draft/` с историей изменений и
готовит место под `clean/`. Дальше файлы кладут в черновик через
...
В прогоне из [README](https://github.com/digitable-lol/ouroboros/blob/main/README.ru.md#4-прочитать-записи)
...
значения; поле `k` заполняет только Python — из именованных доводов. Таблицу по
полям печатает прогон всех восьми: [Языки](languages.ru.md#откуда-берутся-имена-доводов)
— переписанная сюда от руки копия отстала на шести строках, когда языков стало
восемь.
...
Подробнее — [Языки](languages.ru.md#откуда-берутся-имена-доводов).
...
> В обычной программе поле `ci` равно `-1` у **всех восьми** языков, а в разборе —
...
> разбор по языкам — в [Языках](languages.ru.md#ядро-поток-и-часы).
...
Полностью — [Границы](limits.ru.md).
