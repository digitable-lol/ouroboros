...
32 МБ у C# — [Замеры](measurements.ru.md#цена-разборщика-у-java-и-c)), поэтому JDK и
...
<!--state:version-->0.6.1<!--/state-->, с пустого места: `plugin add` по адресу
...
список и что каждый делает — [Чтобы ИИ понимал, как код исполняется](with-ai.ru.md).
...
> `pyproject.toml` и сейчас равна `<!--state:version-->0.6.1<!--/state-->`.
...
- [Начало работы](getting-started.ru.md) — все команды и порядок работы
- [Прологировать чужой код](trace-existing-code.ru.md) — первый настоящий прогон по шагам
- [Границы](limits.ru.md) — прочитать до того, как из трассы вырастут выводы
