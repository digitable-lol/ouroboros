...
- **`th` у всех из двух частей** — процесс и поток, — но вторая часть у каждого
  ...
  у Go номер горутины, у Java номер потока JVM, у C# номер управляемого потока.
...
Диалекты — **решение, а не недоделка**.
...
> как повторить — [Замеры](measurements.ru.md#строгий-режим-javascript).
...
[Границах](limits.ru.md#плата-за-безопасность-строки-в-c-и-c). Есть отдельный облегчённый вид записи для горячих и рекурсивных
...
> прогоном — [Замеры](measurements.ru.md#возврат-списком-в-скобках).
