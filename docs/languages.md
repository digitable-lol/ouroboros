...
Шесть языков. Схема записи у всех одна, а способ вставки и мелкие подробности —
...
## Один и тот же вызов на всех шести
...
- **`a` совпадает у всех шести** — `"2, 3"`, одни значения, без имён. Так было не
...
Короткий ответ: **ниоткуда. Имён доводов в записи нет ни у одного из шести
языков.**

Таблица ниже **напечатана прогоном**, а не написана: один и тот же вызов
`add(2, 3)` собирается и запускается на каждом из шести языков, и сюда попадает
...
**Что это стоит.** У всех шести языков имя позиционного довода из записи не
...
**У всех шести языков в обычной программе `ci` равен `-1`**, а при разборе
...
**Python.** Самый обкатанный из шести. Надстройка ставится **ближе всего к
