...
сборка из исходников, один файл-программа и образ — [Установка](docs/install.ru.md);
...
> [Границах](docs/limits.ru.md#где-обмазка-меняет-поведение-программы). Прогоняйте
...
[Как это выглядит](docs/examples/trace-record.ru.md))*
...
[Начало работы](docs/getting-started.ru.md).
...
→ [Прологировать чужой код](docs/trace-existing-code.ru.md)
...
→ [Чтобы ИИ понимал, как код исполняется](docs/with-ai.ru.md)
...
оговорка внизу страницы, а устройство: [Границы](docs/limits.ru.md).
...
| [`tests/`](tests/) | проверок: <!--state:tests-->1248<!--/state--> |
...
| [`docs/ouroboros.flang`](docs/ouroboros.flang) | тот же инструмент, описанный по-русски программой, которую проверяет компилятор flang |
...
| страница | о чём |
|---|---|
| [Установка](docs/install.ru.md) | uv, Homebrew, asdf, из исходников, подключение сервера MCP |
| [Начало работы](docs/getting-started.ru.md) | все команды, порядок работы, на чём спотыкаются |
| [Прологировать чужой код](docs/trace-existing-code.ru.md) | по шагам: что делать, что видишь, как читать |
| [Чтобы ИИ понимал, как код исполняется](docs/with-ai.ru.md) | сервер MCP и его 17 инструментов |
| [Обычная разработка](docs/in-development.ru.md) | молчащая программа, регрессия, чего не стоит делать |
| [Языки](docs/languages.ru.md) | восемь языков, чем они отличаются в записи |
| [Замеры](docs/measurements.ru.md) | что каждый язык пишет, чего это стоит, как переснять |
| [Границы](docs/limits.ru.md) | чего инструмент не делает и почему это осознанно |
| [В чём смысл](docs/why.ru.md) | зачем нужен и какую работу снимает |
| [Как это выглядит](docs/examples/index.ru.md) | записи, сводка, настройка — целиком |
| [Справочник средств MCP](docs/mcp-tools.ru.md) | все 17 средств, снятые живым разговором с сервером |

Те же страницы опубликованы: <https://digitable-lol.github.io/ouroboros/>.

## Русское описание — это программа

[`docs/ouroboros.flang`](docs/ouroboros.flang) говорит про инструмент то же
самое по-русски, но не прозой, а программой: восемь языков, одиннадцать ключей
записи, две строки на вызов, взаимоисключение `r` и `x`, висящие вызовы, два
способа обмазки, ворота порчи.

Проверяет его компилятор [flang](https://github.com/digitable-lol/flang): типы,
доказанное завершение всех 28 функций и 40 примеров, лежащих внутри самих
функций. Арифметику тоже: на каждый вызов ровно две строки, незакрытых не бывает
меньше нуля, а восемь ключей входа плюс шесть ключей выхода минус одиннадцать
...
```
модуль «Уроборос»: функций 28, из них с доказанным завершением 28; типов 5
docs/ouroboros.flang: проверено — разбор, типы, завершаемость, ядро и примеры; замечаний нет
docs/ouroboros.flang: примеров 40, прошло 40, не прошло 0
```

Написанный документ расходится с делом на следующий день после того, как его
приняли, и расходится молча. Здесь расходиться нечему: соврать в этом файле
значит не собрать его.

## Состояние

Версия <!--state:version-->0.6.0<!--/state-->. Проверки: <!--state:tests-->1248<!--/state--> из <!--state:tests-->1248<!--/state-->, покрытие с ветвями <!--state:coverage_percent-->100<!--/state--> % (`ruff`, `mypy --strict`, `pytest`). Установка
...
объяснением в [Границах](docs/limits.ru.md#где-обмазка-меняет-поведение-программы).
